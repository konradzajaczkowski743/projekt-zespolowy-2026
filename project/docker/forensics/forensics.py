"""
forensics.py
Moduł wykrywania AI-generated / deepfake images.
Wywoływany bezpośrednio z Django (faza testowa).
...
Wynik końcowy: ForensicsResult (dataclass) z polem is_ai_generated + confidence + details.
"""
...
# Modele ładowane raz przy pierwszym wywołaniu — nie przy imporcie modułu.
# Dzięki temu Django startuje szybko, a pamięć alokowana tylko gdy potrzeba.

_models_cache: dict = {}


def _get_device():
    """Auto-detect: CUDA > CPU. MPS (Apple Silicon) pomijamy — Docker Linux."""
    import torch
...
def _load_fast_filter():
    ...
    return _models_cache["fast_filter_processor"], _models_cache["fast_filter"]
...
def _load_primary():
    ...
    return _models_cache["primary_processor"], _models_cache["primary"]
...
def _run_ela(image: Image.Image) -> ELAResult:
    ...
    Metoda: zapisz JPEG z niską jakością, oblicz różnicę z oryginałem.
    Wysoki błąd = potencjalne manipulacje lub generowanie AI.

    Ograniczenia:
    - PNG i WebP konwertujemy do JPEG przed analizą
    - Wielokrotne re-save JPEG może zaburzyć wyniki (kalibracja jakości)
    - Nie jest to samodzielny werdykt — tylko jeden sygnał
    """
    ...
    # % pikseli z błędem > 30 (skala 0-255 po wzmocnieniu x10)
    threshold = 30.0
...
def _run_exif(image: Image.Image) -> EXIFResult:
    """
    Sprawdza metadane EXIF pod kątem:
    - Brak EXIF (podejrzane dla "zdjęcia z aparatu")
    - Software tag wskazujący na AI generator
    - Brak tagu Make/Model (aparat fotograficzny zawsze je ma)
    - Niespójność pól (np. Software = Lightroom ale brak GPS)

    Uwaga: ExifTool odrzucono wcześniej — używamy Pillow._getexif().
    """
    ...
    # Tag IDs: 271=Make, 272=Model, 305=Software, 315=Artist
    TAG_MAKE = 271
    TAG_MODEL = 272
    TAG_SOFTWARE = 305
    ...
    # Brak Make/Model — prawdziwy aparat zawsze ma te pola
    if not make_tag:
        suspicious_fields.append("brak_make")
    if not model_tag:
        suspicious_fields.append("brak_model")

    # Sprawdzenie Software tag
    if software_tag:
...
def _score_to_ai_probability(logits, model_labels: dict) -> tuple[float, str]:
    """
    Normalizuje wyjście modelu do prawdopodobieństwa AI (0.0–1.0).
    Obsługuje różne konwencje labelek między modelami.
    """
    import torch
    ...
    # Szukamy indeksu labelu "AI" / "fake" / "artificial"
    ...
    # Fallback — jeśli nie znaleźliśmy labelek zakładamy idx 0 = AI
    if ai_idx is None:
        ai_idx = 0
...
def _run_ml_ensemble(image: Image.Image) -> MLResult:
    """
    Krok 1: Fast filter (jacoballessio, 11.8M params) — tani pre-screen.
    Krok 2: Primary (Ateeqq SigLIP2) — główna analiza.
    Wynik końcowy: ważona suma obu score'ów.
    """
    import torch
    ...
    ff_inputs = ff_processor(images=img_rgb, return_tensors="pt")
    ...
    pr_inputs = pr_processor(images=img_rgb, return_tensors="pt")
...
def _compute_final_score(
    ...
    Wagi sygnałów:
      ML ensemble    — 70%  (najsilniejszy, bezpośrednia klasyfikacja)
      ELA            — 20%  (artefakty pikselowe)
      EXIF           — 10%  (metadata hint — pomocniczy)

    Zwraca: (is_ai_generated, confidence)
    """
    # ML score — już 0.0–1.0
    ml_signal = ml.ensemble_score

    # ELA score — normalizacja mean_error do 0.0–1.0
    # Empirycznie: mean_error > 15 przy quality=75 jest podejrzany dla AI
    ela_signal = min(ela.mean_error / 25.0, 1.0)

    # EXIF score — binarne sygnały
    exif_signal = 0.0
    if not exif.has_exif:
        exif_signal = 0.4          # brak EXIF jest podejrzany ale nie pewny
    if exif.ai_software_detected:
        exif_signal = 1.0          # bezpośredni dowód
    ...
    # EXIF override — jeśli software tag wskazuje na AI, wynik zawsze pozytywny
    if exif.ai_software_detected:
...
def analyze_image(image: Image.Image) -> ForensicsResult:
    """
    Główna funkcja — wywołuj z Django.

    Przykład użycia w Django view / service:
        from docker.forensics.forensics import analyze_image
    ...
    Przyjmuje: PIL.Image.Image
    Zwraca:   ForensicsResult z polem to_dict() gotowym do JsonResponse
    """
    try:
...
def analyze_image_from_bytes(data: bytes) -> ForensicsResult:
    """Wygodny wrapper dla surowych bajtów (np. z httpx response)."""
    image = Image.open(io.BytesIO(data))
    return analyze_image(image)


def analyze_image_from_path(path: str) -> ForensicsResult:
    """Wygodny wrapper dla ścieżki do pliku (np. testy lokalne)."""
