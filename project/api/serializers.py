...
class VerdictSerializer(serializers.Serializer):
    ...
    authenticity_score = serializers.FloatField()
    predicted_location = serializers.CharField(allow_null=True)
    location_source = serializers.CharField(allow_null=True)
    location_confidence = serializers.FloatField(allow_null=True)
    is_location_consistent = serializers.BooleanField()
    summary = serializers.CharField()


class AuthenticitySerializer(serializers.Serializer):
    is_authentic = serializers.BooleanField()
    confidence_score = serializers.FloatField()
    manipulation_type = serializers.CharField(allow_null=True)
    exif_anomalies = serializers.ListField(child=serializers.CharField())
    noise_analysis = serializers.DictField()


class LocationSerializer(serializers.Serializer):
    predicted_region = serializers.CharField(allow_null=True)
    prediction_source = serializers.CharField(allow_null=True)
    confidence = serializers.FloatField(allow_null=True)
    is_location_consistent = serializers.BooleanField()
    distance_km = serializers.FloatField(allow_null=True)
    streetclip_top5 = serializers.ListField(child=serializers.DictField(), required=False)
    vit_predicted_region = serializers.CharField(allow_null=True, required=False)


class ContentSerializer(serializers.Serializer):
    description = serializers.CharField(allow_blank=True, required=False)
    scenes = serializers.ListField(child=serializers.CharField(), required=False)
    detected_objects = serializers.ListField(child=serializers.DictField())
    described_objects = serializers.ListField(child=serializers.CharField(), required=False)
    main_landmark = serializers.DictField(allow_null=True, required=False)
    license_plates = serializers.ListField(child=serializers.DictField())
...
class AnalysisResultSerializer(serializers.Serializer):
    """
    Reorganized output structure for the ML pipeline.
    This serializer is read-only and maps a custom dictionary from the view.
    """
    verdict = VerdictSerializer()
    authenticity = AuthenticitySerializer()
...
class AnalysisStatusSerializer(serializers.ModelSerializer):
    ...
    # Zmieniamy result na dict, ponieważ widok będzie go dynamicznie składać
