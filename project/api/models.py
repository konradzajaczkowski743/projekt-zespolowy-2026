...
class AnalysisResult(models.Model):
    ...
    image_description: models.JSONField = models.JSONField(
        default=dict,
        blank=True,
        help_text="Detailed image description generated by Gemma LLM when image is authentic.",
    )
    distance_estimation: models.JSONField = models.JSONField(
        default=dict,
        blank=True,
        help_text="Distance estimation data from Depth Anything V2 + object detection.",
    )
...
class ImageQuery(models.Model):
    """
    Stores user questions and AI responses about analyzed images.

    This model enables users to ask questions about a specific analysis result,
    with responses generated by the Gemma LLM.

    Attributes:
        id: UUID primary key.
        result: Reference to the associated AnalysisResult.
        user_query: The question asked by the user about the image.
        ai_response: The answer generated by Gemma LLM.
        status: Current state of the query (PENDING, COMPLETED, FAILED).
        created_at: UTC timestamp when the query was submitted.
        updated_at: UTC timestamp of the last modification.
    """

    class Status(models.TextChoices):
        """Enumeration of allowed states for an image query."""
    ...
    result: models.ForeignKey = models.ForeignKey(
        ...
        help_text="The analysis result this query refers to.",
    )
    user_query: models.TextField = models.TextField(
        help_text="The user's question about the image.",
    )
    ai_response: models.TextField = models.TextField(
        blank=True,
        default="",
        help_text="The AI-generated response to the user's question.",
    )
    status: models.CharField = models.CharField(
        max_length=20,
        choices=Status.choices,
        default=Status.PENDING,
        db_index=True,
        help_text="Current state of the query processing.",
    )
    error_message: models.TextField = models.TextField(
        blank=True,
        default="",
        help_text="Error details if the query processing failed.",
    )
    created_at: models.DateTimeField = models.DateTimeField(auto_now_add=True)
    updated_at: models.DateTimeField = models.DateTimeField(auto_now=True)
    ...
    def __str__(self) -> str:
        """Return a human-readable string representation."""
        return f"ImageQuery(result={self.result_id}, status={self.status})"